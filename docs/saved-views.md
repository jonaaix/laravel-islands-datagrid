...
![The views menu before the first view was saved](/screenshots/view-profiles.webp)
...
```php
class DatatableViewProfile extends Model
{
    protected $fillable = ['public_ref', 'user_id', 'section', 'name', 'payload', 'is_default'];

    protected function casts(): array
    {
        return ['payload' => 'array', 'is_default' => 'boolean'];
    }
}
```

The model class is `App\Models\DatatableViewProfile` by default and can be changed in the
[configuration](/configuration). `section` names the list a view belongs to —
`invoices`, `products` — so one table serves every list.
...
name, an asterisk while `changed`, and an × that resets. The panel lists the user's views
with the default marked by a star, a "Shared with you" entry when the open view belongs
to someone else, and inline naming — Enter confirms, Escape closes, 60 characters at most.
"Save as new" appears while a view is open; the plain save is enabled when there is
something to save.

Every label is translatable through the `labels` prop: `menu`, `save`, `saveAsNew`,
`update`, `rename`, `remove`, `reset`, `shared`, `yours`, `changed`, `placeholder`,
`empty`, `setDefault`, `unsetDefault`, `defaultTitle`. Without it, the strings go through
the translator handed to `provideDatagrid()`.

## Sharing by Link

Because the active reference is state, it is in the URL: `/admin/invoices?view=k3Qz9pL2Xa`.
Anyone who opens the link and may see the list sees the view — read-only, since they do
not own it — and can save a copy of their own from it.

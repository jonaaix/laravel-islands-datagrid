...
answering. Every parameter comes from the URL, and `sort` in particular goes straight into
`orderBy`. Whitelist in the query, and let a bad value fall back rather than fail:
